...
export default function WhisperWithSummary() {
  const [file, setFile] = useState<File | undefined>(undefined);
  const [response, setResponse] = useState<string | undefined>("");
  const [isLoading, setLoading] = useState(false);
  const { askWhisper, askChatGpt } = useOpenAI();
  ...
  const handleClick = async () => {
    try {
      ...
      const rowText: string = await askWhisper(file);
      const messages = [
        {
          role: "system",
          content: "入力された文章の要約と原文をMarkdown形式で返してください。",
        },
        { role: "user", content: rowText },
      ];
      const summary = await askChatGpt(messages);
      setResponse(summary);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  return (
    <>
      <Header
        title="文字起こしして要約する実験室"
        description="Whisper APIを使って音声を文字起こしした結果とその要約を表示します。たぶん"
      />
      <InputSection>
        <>
